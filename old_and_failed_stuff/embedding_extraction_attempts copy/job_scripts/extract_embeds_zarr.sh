#!/bin/bash
#SBATCH --job-name=extract_embeds_zarr.sh
#SBATCH --output=../generated_outputs/extract_embeds_zarr.out  # Save output logs
#SBATCH --error=../generated_outputs/extract_embeds_zarr.err   # Save error logs
...
python3 downstream_tasks/extract_embeds_zarr_dask.py \
  ...
  --zarr_path /scratch/bhole/dvc_data/smoothed/models_4weeks/embeddings_zarr_out \
  --output_dir /scratch/bhole/dvc_data/smoothed/models_4weeks/embeddings_zarr \
