#!/bin/bash
#SBATCH --job-name=ml_extract_embed_short
#SBATCH --output=../generated_outputs/extract_embeds_fixed_short.out  # Save output logs
#SBATCH --error=../generated_outputs/extract_embeds_fixed_short.err   # Save error logs
...
#SBATCH --mem=356G  # Memory requested
#SBATCH --nodes=1  # Run on a single node

export PYTHONPATH="$PWD/src:${PYTHONPATH:-}"

echo "Extracting embeddings..."
python3 downstream_tasks/extract_embeddings_fixed.py \
  ...
  --output_dir /scratch/bhole/dvc_data/smoothed/models_3days/embeds_short \
