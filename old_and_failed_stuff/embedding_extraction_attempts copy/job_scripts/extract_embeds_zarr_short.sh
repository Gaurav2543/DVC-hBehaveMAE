#!/bin/bash
#SBATCH --job-name=extract_embeds_zarr_short.sh
#SBATCH --output=../generated_outputs/extract_embeds_zarr_short.out  # Save output logs
#SBATCH --error=../generated_outputs/extract_embeds_zarr_short.err   # Save error logs
...
python3 downstream_tasks/extract_embeds_zarr_dask.py \
  ...
  --zarr_path /scratch/bhole/dvc_data/smoothed/models_3days/embeddings_zarr_short_out \
  --output_dir /scratch/bhole/dvc_data/smoothed/models_3days/embeddings_zarr_short \
  --time_aggregations 4320 \
  --aggregation_names 3days \
  --batch_size 1024 --multi_gpu \
  --gpu_ids 0 1 2 3 \
  --model hbehavemae \
  --stages 2 3 4 4 5 \
