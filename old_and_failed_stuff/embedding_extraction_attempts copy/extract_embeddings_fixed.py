...
##################################################################################
def build_test_split(root, csv):
    """Modified to match optimized version's filtering"""
    summary_df = pd.read_csv(csv)
    # Apply same head limit as optimized version
    summary_df = summary_df.head(10)  # Match the optimized version
    ...
    # Calculate expected chunk size for validation (like optimized version)
    ...
    for _, r in summary_df.iterrows():
        day = str(r["from_tpt"]).split()[0]
        # Add validation similar to optimized version
        ...
        # Load and validate chunk like optimized version
        ...
        # Apply same strict validation as optimized version
...
#     print(f"Loading model on GPU {device_id}")
#     print(f"GPU {device_id} memory before loading: {torch.cuda.memory_allocated(device_id)/1024**3:.2f} GB")
...
#     del ckpt, model_dict, pretrained_dict
#     torch.cuda.empty_cache()
    
#     print(f"GPU {device_id}: Loaded model, moving to GPU...")
#     model = model.to(device).eval().requires_grad_(False)
    
#     print(f"GPU {device_id} memory after loading: {torch.cuda.memory_allocated(device_id)/1024**3:.2f} GB")
#     return model
...
class SlidingWindowDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure window is always the correct size
        if window.shape[0] < self.num_frames:
            # Pad the window to the correct size
            pad_needed = self.num_frames - window.shape[0]
            window = np.pad(window, ((0, pad_needed), (0, 0)), 'edge')
        
        return torch.from_numpy(window.copy()).unsqueeze(0).unsqueeze(2)  # (1,T,1,12)
...
def main():
    ...
    ##################################################################################
    # Add validation check to match optimized version
    print("Validating aggregation compatibility with data chunks...")
    summary_df_check = pd.read_csv(args.summary_csv)
    summary_df_check = summary_df_check.head(10)  # Same limit
    ...
    for num_frames, agg_name in zip(args.time_aggregations, args.aggregation_names):
        if num_frames % expected_frames_from_summary != 0:
            print(f"WARNING: Aggregation window ({num_frames}) is not a multiple of base chunk size ({expected_frames_from_summary}) for {agg_name}")
            print("This may cause inconsistencies with the optimized version")
    ##################################################################################
