...
def run(args):
    ...
    print(f"🔍 Loaded embeddings → {emb.shape}, fmap → {len(fmap)} entries")
    print("First 10 fmap keys:", list(fmap.keys())[:10])
    ...
    # rename from_tpt to day and take only 1st part of the split by space
    df['day'] = df['from_tpt'].str.split().str[0]
    # rename cage_id to cage
    df.rename(columns={'cage_id': 'cage'}, inplace=True)
    df['day'] = pd.to_datetime(df['day']).dt.date.astype(str)
    # ai,ci,si = df.columns.index('Age_Days'),df.columns.index('cage'),df.columns.index('strain')
    ...
    # 1) Build the set of dates in the CSV:
    csv_dates = set(df['day'].astype(str).unique())

    # 2) Build the set of dates encoded in your fmap keys
    #    assuming your keys look like "EHDP-00083_2019-11-08", etc.
    fmap_dates = { key.split('_')[-1] for key in fmap.keys() }

    # 3) Print any that are missing
    missing = fmap_dates - csv_dates
    if missing:
        print(f"⚠️  These dates from your embeddings map are not in the summary CSV:\n{sorted(missing)}")
    else:
        print("✅ All fmap dates found in summary CSV.")
        
    total_keys = len(fmap)
    empty_chunks = 0
    for key,(st,en) in fmap.items():
        if en <= st:
            empty_chunks += 1
    print(f"Out of {total_keys} keys, {empty_chunks} had en<=st and will be skipped")
    
    # count empty chunks
    empty = sum(1 for st,en in fmap.values() if en <= st)
    print(f"🔢 fmap total keys: {len(fmap)}, skipped (en<=st): {empty}")

    # 4) When you iterate, use only the date part:
    days, Xe, Ya, Ci, Si = [], [], [], [], []
    for key in sorted(fmap):
        ...
        row = df.loc[df['day'] == date_str].iloc[0]

        days.append(key)
        Xe.append(emb[st:en].mean(0))
        Ya.append(row['avg_age_days_chunk_start'])
        Ci.append(row['cage'])
        Si.append(row.get('strain', None))
    ...
    # #     row = df[df['day'] == str(key)].iloc[0]
        
        
    # for chunk_id, key in enumerate(sorted(fmap)):
    #     st, en = fmap[key]
    #     if en <= st:
    #         continue
    ...
    #     # Look up by chunk_nb_overall == chunk_id
    #     sub = df['chunk_nb_overall'] == float(chunk_id)
    #     if not sub.any():
    #         # if you really want to be strict, you can raise here
    #         continue

    #     row = df.loc[sub].iloc[0]
    #     # Pull the age column that makes sense; in your header dump it was
    #     # 'avg_age_days_chunk_start'
    #     Ya.append(row['avg_age_days_chunk_start'])
    ...
    print(f"🏗  Built daily data: {len(days)} samples → X_daily = {X_daily.shape}")
    if len(days) < 10:
        print("❗ Very few samples—here are the days/keys I actually collected:", days)
        
    X_daily = np.vstack(Xe)
    print(f"🏗️  After loop → days={len(days)}, X_daily.shape={X_daily.shape}")
    if len(days) < 5:
        print("❗ Only a handful of samples — here are the `days` I got:", days)
    ...
    s_trv,s_te = s_daily[trv_idx],s_daily[te_idx]
    ...
    plt.ylabel("RMSE (days)"); plt.xlabel("Cage ID")
    ...
    if 'strain' in df.columns:
        ...
        plt.xlabel("Strain"); plt.ylabel("RMSE")
        plt.xticks(rotation=45, ha='right')
        pdf.savefig(); plt.close()
    ...
    if 'strain' in df.columns: metrics["per_strain_rmse"]=strain_rmse
