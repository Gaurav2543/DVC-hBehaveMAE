...
# CONFIG
# EMBEDDINGS_PATH = "test-outputs/test_submission_combined.npy"
...
def plot_umap(embeddings, label_array, vocabulary, label_name, pdf, n_samples=None, remove_legend=False, title_suffix=""):
    ...
    # ------------------------------------------------------------------
    # replace the whole big “if labels.dtype.kind … else …” section
    # ------------------------------------------------------------------
    ax = plt.gca()

    # ---- 1. make sure Age_Days is really numeric --------------------
    try:                                    # strings → float
        labels = labels.astype(float)
    except Exception:
        pass

    if np.issubdtype(labels.dtype, np.number):        # treat as continuous
        ...
        sc = ax.scatter(
            ...
            c=labels, cmap=cmap, norm=norm, s=8, alpha=0.8
        )

        # single colour-bar with 6 evenly-spaced ticks
        cbar = plt.colorbar(sc, ax=ax, pad=0.01)
        cbar.set_ticks(np.linspace(vmin, vmax, 6))
        cbar.set_label(f"{label_name} (min {vmin:.0f} – max {vmax:.0f})")

    else:                                            # categorical fallback
        uniq, enc = np.unique(labels, return_inverse=True)
        sc = ax.scatter(
            embedding_2d[:, 0], embedding_2d[:, 1],
            c=enc, cmap='tab20', s=8, alpha=0.8
        )
        if not remove_legend:
            ax.legend(
                handles=[
                    plt.Line2D([0], [0], marker='o', color='w',
                            markerfacecolor=sc.cmap(sc.norm(i)), label=str(lbl),
                            markersize=6)
