...
def build_test_split(root, csv):
    ...
    summary_df = summary_df.head(58)  # Match the optimized version
...
def load_sequences(root, mapping, order, skip_missing):
    ...
    for cage, df in raw.items():
        ...
        for day, sub in df.groupby("__day"):
            ...
            # Ensure same dtype as optimized version
            seq_data = sub[ELEC_COLS].values.astype(np.float32)
            seqs.append((key, seq_data))
...
def main():
    ...
    summary_df_check = summary_df_check.head(58)  # Same limit
