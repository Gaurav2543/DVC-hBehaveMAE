#!/bin/bash
#SBATCH --job-name=n_extract_embeds_zarr.sh
#SBATCH --output=../generated_outputs/extract_embeds_zarr_new.out  # Save output logs
#SBATCH --error=../generated_outputs/extract_embeds_zarr_new.err   # Save error logs
...
python3 downstream_tasks/extract_embeds_zarr_dask_new.py \
  ...
  --output_dir /scratch/bhole/dvc_data/smoothed/models_4weeks/embeddings_zarr_new \
  --zarr_path /scratch/bhole/dvc_data/smoothed/models_4weeks/embeddings_zarr_new_out \
