...
def train_one_epoch(
    ...
    # added code
    total_train_loss = 0.0  # Accumulator for train loss
    num_train_batches = 0   # Counter for train batches
    ...
    for data_iter_step, samples in enumerate(
        ...
        #added code
        total_train_loss += loss_value  # Accumulate train loss
        num_train_batches += 1         # Increment train batch counter
    ...
    if log_writer is not None:
        log_writer.add_scalar("avg_train_loss", avg_train_loss, epoch)

    if data_loader_val:

        # added code
        total_test_loss = 0.0  # Accumulator for test loss
        ...
        if epoch % 5 == 0:
            for data_iter_step, samples in enumerate(
                ...
                # added code
                total_test_loss += loss_value  # Accumulate test loss
                num_test_batches += 1         # Increment test batch counter
            ...
            if log_writer is not None:
                log_writer.add_scalar("avg_test_loss", avg_test_loss, epoch)
