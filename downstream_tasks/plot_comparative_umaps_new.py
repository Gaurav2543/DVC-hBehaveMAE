...
# Argument Parsing
# =============================
parser = argparse.ArgumentParser(description="UMAP Visualization Script with Strain Filtering (Zarr-based)")
...
parser.add_argument('--aggregation', required=True,
                    help='Aggregation level to use (e.g., "3days", "1day")')
parser.add_argument('--embedding_levels', required=True, nargs='+',
                    help='List of embedding levels (e.g., level_1_pooled level_2_pooled combined)')
parser.add_argument('--embedding_labels', required=True, nargs='+',
                    help='List of embedding labels (must match number of embedding_levels)')
parser.add_argument('--embedding_lengths', required=True, help='JSON string: {"level1":"(5mins)",...}')
...
# Validate that embedding_levels and embedding_labels have same length
if len(args.embedding_levels) != len(args.embedding_labels):
    raise ValueError("Number of embedding_levels must match number of embedding_labels")
...
for label in args.embedding_labels:
    for method in ["mean", "median", "percentiles"]:
        ...
        # Adjust json filename pattern as needed
        json_filename = f"{args.aggregation}_{label}.json"
...
# =============================
# Utilities - ADAPTED FOR ZARR
# =============================
def load_embeddings_from_zarr(embeddings_dir, aggregation, embedding_level, metadata):
    """
    Load embeddings from Zarr stores for all sequences in metadata.
    Returns: embeddings dict mapping key -> embedding array
    """
    ...
    print(f"[INFO] Loading {embedding_level} embeddings from {core_emb_dir}...")
    
    for _, row in tqdm(metadata.iterrows(), total=len(metadata), 
                      desc=f"Loading {embedding_level}", disable=not sys.stdout.isatty()):
        key = row['key']
        ...
        if zarr_path.exists():
            try:
                ...
                if embedding_level not in store:
                    print(f"[WARN] Level '{embedding_level}' not in {zarr_path}, trying 'combined'")
                    level_to_use = 'combined'
                else:
                    level_to_use = embedding_level
                
                embedding = store[level_to_use][:]
                
                # Store the full temporal sequence
                embeddings_dict[key] = embedding
                
            except Exception as e:
                print(f"[WARN] Failed to load {zarr_path}: {e}")
    
    print(f"[INFO] Loaded {len(embeddings_dict)} sequences for {embedding_level}")
...
# =============================
# Load embeddings from Zarr - ADAPTED
# =============================
embedding_types = {}
for level, label in zip(args.embedding_levels, args.embedding_labels):
    embedding_types[label] = level

datasets = {}

for label, embedding_level in tqdm(embedding_types.items(), desc="[LOOP] Embedding types", disable=not sys.stdout.isatty()):
    print(f"\n[INFO] Processing {label} embeddings (level: {embedding_level})...")
    
    # Load all embeddings for this level
    embeddings_dict = load_embeddings_from_zarr(args.embeddings_dir, args.aggregation, 
                                               embedding_level, metadata)
    ...
    for method in ["mean", "median", "percentiles"]:
        ...
        for row in metadata.itertuples():
            ...
            if emb.ndim == 1:
                # Already aggregated daily - use as-is for all methods
                flat = emb
            else:
                # Has temporal dimension - aggregate according to method
