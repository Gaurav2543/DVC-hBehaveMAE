...
class TemporalSubAggregationAnalyzer:
    ...
    def __init__(self, embeddings_path, metadata_path, output_dir="temporal_subaggregation_results", 
                 max_workers=8, bodyweight_files=None, aggregation='1day', embedding_level='combined'):
    ...
    def load_data(self):
        ...
        self.core_emb_dir = self.embeddings_path / self.aggregation / 'core_embeddings'
        
        if not self.core_emb_dir.exists():
            raise FileNotFoundError(f"Embeddings directory not found: {self.core_emb_dir}")
        
        print("Loading metadata...")
        self.metadata = pd.read_csv(self.metadata_path)
        
        # Get aggregation info from sample
        sample_zarr = next(self.core_emb_dir.glob('*.zarr'), None)
        if sample_zarr:
            ...
            if 'timestamps' in sample_store:
                ...
                self.aggregation_info = {'num_frames': 1440, 'aggregation_name': self.aggregation}
        ...
        # Create temporal aligned dataset
        self.create_temporal_aligned_dataset_zarr()
    
    def create_temporal_aligned_dataset_zarr(self):
        """Create temporally aligned dataset with sub-aggregations from Zarr stores"""
        print("Creating temporal sub-aggregation dataset from Zarr stores...")
        ...
        for idx, row in tqdm(self.metadata.iterrows(), total=len(self.metadata), desc="Processing sequences"):
            ...
            if zarr_path.exists():
                try:
                    ...
                    # Only process if we have temporal dimension
                    if seq_embeddings.ndim > 1:
                        ...
                        print(f"[WARN] {key} has no temporal dimension, skipping")
...
def main():
    ...
    parser.add_argument('--aggregation', default='1day', 
                       help='Aggregation level to use (e.g., "1day", "1.5h")')
