...
def extract_embeddings_for_aggregation(model, sequences, num_frames, batch_size, device, num_workers):
    ...
    frame_map, ptr = {}, 0
    # LOW, MID, HIGH, COMB = [], [], [], []
    LEVEL1, LEVEL2, LEVEL3, LEVEL4, LEVEL5, COMB = [], [], [], [], [], []
    pad = (num_frames - 1) // 2
    ...
    for key, mat in tqdm(sequences, desc=f"Processing {num_frames}f", unit="seq"):
        ...
        with torch.no_grad():
            for batch in tqdm(dl, desc="Batches", leave=False):
                ...
                # l, l_m, m, m_h, h = levels
                l, l_m, m, m_h, h = levels[0], levels[1], levels[2], levels[3], levels[4]

                def pool(feat):  # (B, T, H, W, C) → (B,C)
                    return feat.flatten(1, -2).mean(1).float().cpu()

                level1.append(pool(l))
                level2.append(pool(l_m))
                level3.append(pool(m))
                level4.append(pool(m_h))
                level5.append(pool(h))
                
        level1 = torch.cat(level1).numpy().astype(np.float16)
        level2 = torch.cat(level2).numpy().astype(np.float16)
        level3 = torch.cat(level3).numpy().astype(np.float16)       
        level4 = torch.cat(level4).numpy().astype(np.float16)
        level5 = torch.cat(level5).numpy().astype(np.float16)
        comb = np.concatenate([level1, level3, level5], axis=1)

        LEVEL1.append(level1)
        LEVEL2.append(level2)
        LEVEL3.append(level3)
        LEVEL4.append(level4)
        LEVEL5.append(level5)
        COMB.append(comb)
    ...
    return {
        'level1': np.concatenate(LEVEL1, axis=0),
        'level2': np.concatenate(LEVEL2, axis=0),
        'level3': np.concatenate(LEVEL3, axis=0),
        'level4': np.concatenate(LEVEL4, axis=0),       
        'level5': np.concatenate(LEVEL5, axis=0), 
...
def main():
    ...
    for num_frames, agg_name in zip(args.time_aggregations, args.aggregation_names):
        ...
        for level in ['level1', 'level2', 'level3', 'level4', 'level5', 'comb']:
