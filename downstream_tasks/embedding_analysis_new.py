...
class EmbeddingAnalyzer:
    def __init__(self, embeddings_path, metadata_path, output_dir="analysis_results", 
                 max_workers=8, aggregation='1day', embedding_level='combined'):
        """
        Initialize the embedding analyzer.
        
        Args:
            embeddings_path: Path to embeddings directory (contains aggregation subdirs)
            metadata_path: Path to metadata CSV file
            output_dir: Directory to save results
            max_workers: Number of threads for parallel processing
            aggregation: Aggregation level to analyze (e.g., '1day', '1.5h')
            embedding_level: Which embedding level to use ('level_1_pooled', 'combined', etc.)
        """
    ...
    def load_data(self):
        """Load embeddings from Zarr stores and metadata"""
        ...
        if sample_zarr:
            ...
            # Infer number of frames from timestamps if available
            if 'timestamps' in sample_store:
                ...
                # Fallback
                self.aggregation_info = {
                    'num_frames': 1440,  # Assume 1 day = 1440 minutes
                    'aggregation_name': self.aggregation
                }
        else:
    ...
    def create_aligned_dataset_zarr(self):
        ...
        aligned_embeddings = []
        aligned_metadata = []
        frame_map = {}  # Create a frame_map for compatibility with heatmap methods
        ...
        for idx, row in tqdm(self.metadata.iterrows(), total=len(self.metadata), desc="Aligning data"):
            ...
            if zarr_path.exists():
                try:
                    ...
                    # Store full temporal sequence for heatmaps
                    if seq_embeddings.ndim == 1:
                        # Already aggregated daily
                        daily_embedding = seq_embeddings
                        # Can't create heatmaps without temporal dimension
                        n_frames = 1
                    else:
                        ...
                        # Store in frame_map for heatmap compatibility
                        frame_map[key] = (current_frame_idx, current_frame_idx + n_frames)
        ...
        self.frame_map = frame_map  # For heatmap methods
    ...
    def create_activity_heatmaps(self, n_samples=100):
        ...
        # Get available sequences with temporal data
        available_keys = list(self.frame_map.keys())[:n_samples]
        
        if not available_keys:
            print("[WARN] No temporal data available for heatmaps. Skipping.")
            return
        
        # Load temporal embeddings from Zarr stores
        n_dims = self.X.shape[1]
        frames_per_day = self.aggregation_info['num_frames']
        
        for dim in tqdm(range(min(16, n_dims)), desc="Creating heatmaps"):
            ...
            for key in available_keys:
                ...
                if zarr_path.exists():
                    try:
                        ...
                        if seq_embeddings.ndim > 1 and seq_embeddings.shape[0] == frames_per_day:
                            seq_data = seq_embeddings[:, dim]
        ...
        # Create average activity patterns
        self.create_average_activity_patterns_zarr(available_keys)
    
    def create_average_activity_patterns_zarr(self, sample_keys):
        """Create average activity patterns across dimensions using Zarr stores"""
            ...
            for key in sample_keys:
                ...
                if zarr_path.exists():
                    try:
                        ...
                        level_to_use = self.embedding_level if self.embedding_level in store else 'combined'
                        seq_embeddings = store[level_to_use][:]
