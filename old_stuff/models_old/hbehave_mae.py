...
class HBehaveMAE(GeneralizedHiera):
    ...
    def forward_loss(
        ...
        # plt.imshow(pred.detach().cpu().numpy().reshape(-1, pred.shape[-1])[:200,:200], cmap='viridis')
        # plt.colorbar()
        # plt.subplot(1, 2, 2)
        # plt.title("Labels")
        # plt.imshow(label.detach().cpu().numpy().reshape(-1, label.shape[-1])[:200,:200], cmap='viridis')
        # plt.colorbar()
        # plt.tight_layout()
        # filename = os.path.join(output_dir, f"predictions_labels_plot_{timestamp}.png")
