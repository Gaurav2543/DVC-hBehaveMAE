...
#SBATCH --output=../generated_outputs/3d_umap_anal_pred.out
#SBATCH --error=../generated_outputs/3d_umap_anal_pred.err
...
# === Define parameters ===
RESULTS_JSON_PREFIX="4320_${embedding_aggregation}"
...
# Pass a dummy file in RESULTS_ROOT so umap_generation.py uses that root
DUMMY_JSON="$RESULTS_ROOT/dummy.json"
touch "$DUMMY_JSON"

python3 -u downstream_tasks/umap_generation.py \
  --metadata_path "$METADATA_PATH" \
  --results_json "$DUMMY_JSON" \
