#!/bin/bash
#SBATCH --job-name=3d_e_embeds
#SBATCH --output=../generated_outputs/3d_embeds.out  # Save output logs
#SBATCH --error=../generated_outputs/3d_embeds.err   # Save error logs
...
python3 downstream_tasks/extract_embeddings_multi.py --dvc_root /scratch/bhole/dvc_data/smoothed/cage_activations_full_data_final \
    ...
    --aggregation_names 3days --batch_size 192 \
    --model hbehavemae \
